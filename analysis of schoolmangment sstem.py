# -*- coding: utf-8 -*-
"""
Created on Fri Dec 31 06:31:32 2021
...
connector = ps.connect(host='localhost',
    database='myschool',
    user='postgres',
    password='12321',
    port='5432'
    )
#qury="CREATE TABLE teacher ( NAME VARCHAR, EMAIL VARCHAR, PHONE_NO VARCHAR, GENDER VARCHAR, DOB VARCHAR, STREAM  VARCHAR  )"
...
def display_records():
   tree.delete(*tree.get_children())
   curr.execute('SELECT * FROM teacher ')
   data = curr.fetchall()
   for records in data:
...
def add_record():
  ...
  # curr.execute("INSERT INTO SCHOOL_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES(name, email, contact, gender, DOB, stream)")
  # curr.commit()  
   if not name or not email or not contact or not gender or not DOB or not stream:
       mb.showerror('Error!', "Please fill all the missing fields!!")
   else:
       
           curr.execute("INSERT INTO teacher(NAME,EMAIL,PHONE_NO,GENDER,DOB,STREAM) " "VALUES (%s, %s, %s, %s, %s, %s)",(name, email,contact,gender,DOB,stream ))
           connector.commit()
...
def remove_record():
   if not tree.selection():
       ...
       curr.execute('DELETE FROM teacher  WHERE STUDENT_ID=%d' % selection[0])
       connector.commit()
...
main = Tk()
main.title(' OXFORD PUBLIC HIGH SCHOOL PAKHNAHA BAZAR')
...
Label(main, text="OXFORD PBUBLIC HIGH SCHOOL PAKHNAHA       Created by :- omprakash kumar", font=headlabelfont, bg='yellow').pack(side=TOP, fill=X)
...
tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                 #  columns=('Student ID', "Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "Stream"))
                   columns=("Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "Stream"))
